...
export default function ProductCard({ product, index = 0 }: ProductCardProps) {
  ...
  // Update current image when showBackImage changes or when product changes
  useEffect(() => {
    setCurrentImage(showBackImage ? product.backImage : product.frontImage)
  }, [showBackImage, product.frontImage, product.backImage])
    ...
    >
      <div className="relative aspect-square bg-background overflow-hidden mb-2 transition-transform duration-300 group-hover:scale-[1.02] shadow-sm">
        ...
        <button
          ...
          className="absolute top-2 right-2 h-8 w-8 rounded-full bg-white/80 backdrop-blur-sm text-gray-800 flex items-center justify-center hover:bg-white transition-colors z-30 group-hover:animate-pulse"
      ...
      <div className="relative z-40">
        ...
        <p className="font-medium text-orange-500">$ {product.price.toFixed(2)}</p>
      </div>
      <div className="flex gap-1 mt-1 relative z-40">
        {product.colors.slice(0, 4).map((color:any, index:any) => (
